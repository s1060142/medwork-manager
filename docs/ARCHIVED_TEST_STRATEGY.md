...
- Grid sync: 5/5 PASS (estensione pianificata)
...
### Esempio: Nuova Feature Protocol Groups
- [ ] Backend: `dotnet test` per CompanyGroupsIntegrationTests
- [ ] Frontend: `npm run build` senza errori
- [ ] Playwright: `grid-sync-companies.spec.ts` o nuovo spec
- [ ] Tenant isolation: verifica che i gruppi siano per tenant
- [ ] Regressione: tutti i test Playwright esistenti passano
...
| **EF Core Migrations** | Test database schema |
| **BCrypt.Net** | Test sicurezza password |

---

## 📝 Note Importanti

1. **Test data**: tutti i test assumono dati seedati. Usare API calls in `beforeEach` per creare dati freschi.
2. **Tenants**: test che richiedono multi-tenant isolamento devono creare tenant temporanei via API.
3. **Cleanup**: ogni test deve pulire i dati creati per evitare inquinamento dei run successivi.
4. **Stabilità**: usare `waitForSelector` con timeout generosi (10-30s) per operazioni di rete.
5. **Locators**: preferire `getByRole`, `getByLabel`, `getByText` rispetto a selettori CSS per resilienza.
