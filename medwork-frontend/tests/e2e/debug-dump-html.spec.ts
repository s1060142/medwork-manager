...
test('debug dump html', async ({ page }) => {
  ...
  // Test getByLabel
  const byLabel = dialog.getByLabel('Ruolo nel Gruppo')
  console.log('getByLabel count:', await byLabel.count())

  // Test getByRole combobox
  const byRole = dialog.getByRole('combobox')
  console.log('getByRole combobox count:', await byRole.count())

  // Check label association
  const labels = await dialog.locator('label').all()
  for (const l of labels) {
    const text = await l.textContent()
    const htmlFor = await l.getAttribute('for')
