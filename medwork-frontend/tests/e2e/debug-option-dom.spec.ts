...
test('debug option dom', async ({ page }) => {
  ...
  // Dump outerHTML of each option
  const opts = await page.locator('[role="listbox"] [role="option"]').all()
  console.log('Options count:', opts.length)
  for (let i = 0; i < opts.length; i++) {
    const html = await opts[i].evaluate(el => el.outerHTML)
    console.log('OPT ' + i + ' HTML: ' + html.substring(0, 400))
    const text = await opts[i].textContent()
    console.log('OPT ' + i + ' text: "' + text + '"')
  }

  // Test getByRole option name
  const opt = dialog.getByRole('option', { name: 'Medico Competente Principale' })
  console.log('getByRole option count:', await opt.count())

  // Test text-based
  const optText = page.getByText('Medico Competente Principale').first()
  console.log('getByText count:', await page.getByText('Medico Competente Principale').count())
})
